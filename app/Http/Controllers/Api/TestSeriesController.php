...
class TestSeriesController extends Controller
{
    ...
    public function testSeriesList(Request $request)
    {
        ...
        $inviteArray = [];
        $inviteArray1 = [];
        $result = TestSeries::where("user_id", $request->user_id)->orWhere("is_approve", 2)->select('id', 'name', 'total_question', 'created_at')->latest()->get();
        ...
        $result1 = UserTestSeries::where("user_id", $request->user_id)->select('id', 'name', 'is_attempted', 'created_at')->latest()->get();
    ...
    public function testSeriesDetails(Request $request)
    {
        ...
        if ($request->flag == 2) {
            ...
            foreach ($series as $k => $ser) {
                $que = Question::where('id',$ser->question_id)->first();
                if($que){
                    $minut = $minut + $que->ques_time;
                }
